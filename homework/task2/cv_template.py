        ...
        while not self.stopevent.is_set():
            if not self.stopflag:
                ...
                if mode.get() == BINARY:
                    ...
                    binary = cv.inRange(gray, lThreshold, hThreshold)
                    ...
                    # Add your code here
                    gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
                    prev_gray = cv.cvtColor(self.buffer, cv.COLOR_BGR2GRAY)
                    diff = cv.absdiff(gray, prev_gray)
                    self.buffer = frame.copy()  # Save original frame, not diff
                    frame = cv.cvtColor(diff, cv.COLOR_GRAY2BGR)
                elif mode.get() == RGB:
                    lThreshold = Slider1.get()
                    hThreshold = Slider2.get()
                    # Add your code here
                    ...
                    # Binary mask between thresholds
                    thresh = cv.inRange(gray, lThreshold, hThreshold)
                    ...
                    for cnt in contours:
                        if cv.contourArea(cnt) > 500:  # filter noise
